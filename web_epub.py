...
class EbookPolisher:
    ...
    # --- 静态方法：SVG 纹样库 (已去除所有 border 灰线) ---
    @staticmethod
    def generate_decoration_html(title, style, color):
        # 共同的居中容器样式：宽度100%，纯净无边框
    ...
    def reconstruct_chapter(self, content):
        ...
        for type_, content in merged_nodes:
            if type_ == 'text':
                ...
                # 如果开启首字下沉，且是第一段，且内容足够长
                if first_para and self.config.get('enable_dropcaps', False) and len(fixed) > 1:
                    # 创建 span 包裹第一个字
                    span = new_soup.new_tag("span", attrs={'class': 'drop-cap'})
                    span.string = fixed[0]
                    p.append(span)
                    p.append(fixed[1:]) # 剩下的字
                    first_para = False
                else:
                    p.string = fixed
    ...
    def process(self):
        ...
        # V15.0: 物理首字下沉 CSS (Span Selector)
        dropcap_css = ""
...
# ==========================================
# 5. Streamlit 界面 (V15.0 Final)
# ==========================================
st.set_page_config(page_title="电子书精排 V15.0", page_icon="🎨", layout="centered")

if 'processed_path' not in st.session_state:
    st.session_state.processed_path = None

# --- 上传区域 ---
st.title("📚 电子书精排工具 V15.0")
...
with st.expander("🎨 点击展开/折叠 排版效果预览", expanded=False):
    ...
    if enable_dropcaps:
        ...
        <style>
        span.drop-cap {
            font-size: 3.5em; font-weight: bold; float: left;
            line-height: 0.8; margin-right: 0.1em; margin-top: 0.1em; color: inherit;
        }
        </style>
        """
        demo_text_html = f'<span class="drop-cap">{first_char}</span>{rest_text}'
    else:
    ...
    {dropcap_css}
    {demo_title_html}
    <p style="{demo_p_style}">{demo_text_html}</p>
